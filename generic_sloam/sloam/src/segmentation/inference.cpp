...
namespace seg {
Segmentation::Segmentation(const std::string modelFilePath, const float fov_up,
                           const float fov_down, const int img_w,
                           const int img_h, const int img_d, bool do_destagger,
  ...
  _fov = std::abs(_fov_down) +
         std::abs(_fov_up);  // get field of view total in radians
  _img_w = img_w;
  _img_h = img_h;
  _img_d = img_d;
  _verbose = false;
  ...
  if (run_segmentation) {
    ...
    std::cout << "not running segmentation on SLOAM side"
              << "\n";
  }
}

void Segmentation::_startONNXSession(const std::string sessionName,
                                     const std::string modelFilePath,
                                     bool useCUDA, size_t numThreads) {
  ...
  auto env = boost::make_shared<Ort::Env>(ORT_LOGGING_LEVEL_ERROR,
                                          sessionName.c_str());
  _env = boost::move(env);

  auto session = boost::make_shared<Ort::Session>(*_env, modelFilePath.c_str(),
                                                  sessionOptions);
  ...
  std::cout << "[Segmentation] Input Dimensions: ";
  printVector(_inputDims);
  ...
  std::cout << "[Segmentation] Output Dimensions: ";
  printVector(_outputDims);
...
void Segmentation::_makeTensor(std::vector<std::vector<float>>& projected_data,
...
void Segmentation::destaggerCloud(const Cloud::Ptr cloud,
                                  Cloud::Ptr& outCloud) {
  bool col_valid = true;
...
void Segmentation::maskCloud(const Cloud::Ptr cloud, cv::Mat mask,
                             Cloud::Ptr& outCloud, unsigned char val,
                             bool dense) {
  ...
  if (dense) {
    ...
    if (_do_destagger) {
      destaggerCloud(tempCloud, outCloud);
      ROS_INFO_THROTTLE(1, "destaggering cloud");
    } else {
      outCloud = tempCloud;
    }
...
void Segmentation::_mask(const float* output,
                         ...
                         cv::Mat& maskImg, const int& tree_label) {
  ...
  for (int pixel_id = 0; pixel_id < channel_offset; pixel_id++) {
    ...
    if (out_idx == tree_label) out_idx = 255;
...
void Segmentation::_preProcessRange(const cv::Mat& img, cv::Mat& pImg,
                                    float maxDist) {
  ...
  cv::multiply(pImg, cv::Scalar(rangeMultiplier, intensityMultipler), pImg);
}
...
void Segmentation::runERF(cv::Mat& rImg, cv::Mat& maskImg) {
  ...
  inputTensors.push_back(Ort::Value::CreateTensor<float>(
      *_memoryInfo, inputTensorValues.data(), _inputTensorSize,
      _inputDims.data(), _inputDims.size()));
...
void Segmentation::run(const Cloud::Ptr cloud, cv::Mat& maskImg,
  ...
  float* outData = outputTensors.front().GetTensorMutableData<float>();
  _mask(outData, invalid_idxs, maskImg, tree_label);
