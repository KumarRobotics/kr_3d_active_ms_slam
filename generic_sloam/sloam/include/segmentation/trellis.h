...
class Instance {
 public:
   ...
   */
  void computeGraph(const CloudT::Ptr tree_cloud,
                    std::vector<std::vector<TreeVertex>> &landmarks);

  bool computeVertexProperties(CloudT::Ptr &pc, Slash &filteredPoints,
 ...
 private:
  ...
  /**
   * @brief computeMedianAndFilterPoints_: Remove outlier points in vertex based
   * on the median. Also returns the median.
   * @param vertexPoints: vector of points in vertex
   */
  void computeMedianAndFilterPoints_(VectorType &vertexPoints,
                                     PointT &median_point);
