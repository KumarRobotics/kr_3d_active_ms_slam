...
public:
  ...
  static double computePathTimeCost(const Vector3d& p1, const Vector3d& p2, const double& y1, const double& y2,
                             const Vector3d& v1, const double& yd1, vector<Vector3d>& path);
  static double computePathCost(const Vector3d& p1, const Vector3d& p2, vector<Vector3d>& path);
  // Coarse to fine path searching
  static double searchPath(const Vector3d& p1, const Vector3d& p2, vector<Vector3d>& path);

  static double searchRawPath(const Vector3d& p1, const Vector3d& p2, vector<Vector3d>& path);
  ...
  inline static std::mutex astar_mutex_;
  // Or use boost::mutex ViewNode::astar_mutex_ in .cpp;
};
}
#endif