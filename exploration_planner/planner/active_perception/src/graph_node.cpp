...
double ViewNode::searchRawPath(const Vector3d& p1, const Vector3d& p2, vector<Vector3d>& path) {
  astar_mutex_.lock();
  // Try connect two points with straight line
  bool safe = true;
...
double ViewNode::computePathTimeCost(const Vector3d& p1, const Vector3d& p2, const double& y1, const double& y2,
                             const Vector3d& v1, const double& yd1, vector<Vector3d>& path) {
  // Cost of position change
  double pos_cost = ViewNode::searchPath(p1, p2, path) / vm_;
...
double ViewNode::computePathCost(const Vector3d& p1, const Vector3d& p2, vector<Vector3d>& path) {
  // Cost of position change
  double pos_cost = ViewNode::searchPath(p1, p2, path);
  return pos_cost;
}
