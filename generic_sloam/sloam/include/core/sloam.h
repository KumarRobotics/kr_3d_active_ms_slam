...
class sloam {
 public:
  ...
  // Pose Optimization
  void OptimizeXYYaw(const SE3 &poseEstimate, const bool optimize,
                     const std::vector<ObjectMatch<Cylinder>> &allTMatch,
                     double *);
  void OptimizeZRollPitch(const SE3 &poseEstimate, const bool optimize,
                          const std::vector<ObjectMatch<Plane>> &allGMatch,
                          double *);
  bool TwoStepOptimizePose(const SE3 &poseEstimate, const bool optimTrees,
                           const bool optimGround,
                           const std::vector<ObjectMatch<Cylinder>> &allTMatch,
                           const std::vector<ObjectMatch<Plane>> &allGMatch,
                           SE3 &tf);
  ...
  // Model estimation
  void projectModels(const SE3 &tf, std::vector<Cylinder> &landmarks,
                     const std::vector<Cube> &scan_cubes_body,
                     std::vector<Cube> &scan_cubes_world,
                     std::vector<Plane> &planes);
  void projectModels(const SE3 &tf, const std::vector<Cube> &scan_cubes_body,
                     std::vector<Cube> &scan_cubes_world,
                     std::vector<Plane> &planes);
  void projectModels(const SE3 &tf, const std::vector<Cube> &scan_cubes_body,
                     std::vector<Cube> &scan_cubes_world);
  void computeModels(SloamInput &in, std::vector<Cylinder> &landmarks,
                     std::vector<Plane> &planes);
  void binGroundPoints(const SE3 pose, const VectorType &points,
