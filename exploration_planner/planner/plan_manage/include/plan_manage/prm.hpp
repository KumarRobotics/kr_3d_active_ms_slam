...
#include <plan_env/raycast.h>
  ...
  void init(ros::NodeHandle& nh) {
    ...
    resolution_ = edt_environment_->occ_map_->getResolution();
    Eigen::Vector3d origin, size;
    edt_environment_->occ_map_->getRegion(origin, size);
  ...
  bool lineVisib(const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, double thresh,
    ...
    while (casters_[caster_id]->step(ray_pt)) {
      ...
      dist = edt_environment_->occ_map_->getDistance(pt_id);
      if (dist <= thresh) {
        edt_environment_->occ_map_->indexToPos(pt_id, pc);
        return false;
      }
    }
    return true;
  ...
  void pathToGuidePts(vector<Eigen::Vector3d>& path, const int& pt_num,
                                  vector<Eigen::Vector3d>& pts) {
    pts = discretizePath(path, pt_num);
  }

  void shortcutPath(vector<Eigen::Vector3d> path, int path_id, int iter_num) {
  ...
  void shortcutPaths() {
    ...
    if (parallel_shortcut_) {
      ...
      for (int i = 0; i < raw_paths_.size(); ++i) {
        short_threads.push_back(thread(&shortcutPath, this, raw_paths_[i], i, 1));
      }
      for (int i = 0; i < raw_paths_.size(); ++i) {
