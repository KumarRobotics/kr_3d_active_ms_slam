...
ros::Publisher vioPosePub;
ros::Publisher vioPathPub;
ros::Publisher vioVelPub;
ros::Publisher vioCovPub;
ros::Publisher vioCovVelPub;
ros::Publisher vioTrajPub;
ros::Publisher sensorPub;
ros::Publisher meshPub;
ros::Publisher heightPub;
ros::Publisher fov_pub_;
ros::Time odom_start_time_;
bool init_time_;
double traj_len_;
string out_fname_;
...
visualization_msgs::Marker vioTrajROS;
...
void fov_visual_init(std::string msg_frame_id) {
  ...
  markerEdge_fov.color.r = color_r;
  markerEdge_fov.color.g = color_g;
  markerEdge_fov.color.b = color_b;
  markerEdge_fov.color.a = 0.8;
}
...
void vio_odom_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  if ((t - pt).toSec() > 0.5)
  {
    vioTrajROS.header.frame_id = _frame_id;
    vioTrajROS.header.stamp    = ros::Time::now();
    vioTrajROS.ns              = string("vio_trajectory");
    vioTrajROS.type            = visualization_msgs::Marker::LINE_LIST;
    vioTrajROS.action          = visualization_msgs::Marker::ADD;  
    vioTrajROS.pose.position.x    = 0;
    vioTrajROS.pose.position.y    = 0;
    vioTrajROS.pose.position.z    = 0;  
    vioTrajROS.pose.orientation.w = 1;  
    vioTrajROS.pose.orientation.x = 0;  
    vioTrajROS.pose.orientation.y = 0;  
    vioTrajROS.pose.orientation.z = 0;
    vioTrajROS.scale.x = 0.05;
    vioTrajROS.scale.y = 0;
    vioTrajROS.scale.z = 0;
    vioTrajROS.color.r = 0.0;
    vioTrajROS.color.g = 1.0;
    vioTrajROS.color.b = 0.0;
    vioTrajROS.color.a = 0.8;  
    geometry_msgs::Point p;          
    p.x = ppose(0);
    p.y = ppose(1);
    p.z = ppose(2);
    vioTrajROS.points.push_back(p);   
    p.x = pose(0);
    p.y = pose(1);
    p.z = pose(2);
    vioTrajROS.points.push_back(p);    
    ...
    color.b = 0;
    color.a = 1;
    vioTrajROS.colors.push_back(color);
    vioTrajROS.colors.push_back(color);

    // Compute traj len and save
    double time_now = (ros::Time::now() - odom_start_time_).toSec();
    traj_len_ += sqrt(std::pow(ppose(0) - pose(0), 2) + std::pow(ppose(1) - pose(1), 2) + std::pow(ppose(2) - pose(2), 2));
    
    ppose = pose;
    pt = t;
    vioTrajPub.publish(vioTrajROS);
  }
}
...
void cmd_callback(const kr_mav_msgs::PositionCommand cmd)
{   
...
int main(int argc, char** argv)
{
  ...
  n.param("odom_vis/out_fname", out_fname_,  string("/home/yuezhan/benchmark"));    
  init_time_ = false;
  traj_len_ = 0.0;
  ros::Subscriber sub_slam_odom = n.subscribe("slam_odom", 100,  slam_odom_callback);
  ros::Subscriber sub_vio_odom = n.subscribe("vio_odom", 100,  vio_odom_callback);
  ...
  vioPosePub   = n.advertise<geometry_msgs::PoseStamped>("vio_pose",                100, true);
  vioPathPub   = n.advertise<nav_msgs::Path>(            "vio_path",                100, true);
  vioVelPub    = n.advertise<visualization_msgs::Marker>("vio_velocity",            100, true);
  vioCovPub    = n.advertise<visualization_msgs::Marker>("vio_covariance",          100, true);
  vioCovVelPub = n.advertise<visualization_msgs::Marker>("vio_covariance_velocity", 100, true);
  vioTrajPub   = n.advertise<visualization_msgs::Marker>("vio_trajectory",          100, true);
