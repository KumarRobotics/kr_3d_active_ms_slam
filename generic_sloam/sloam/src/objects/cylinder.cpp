...
Cylinder::Cylinder(const std::vector<TreeVertex> vertices, const Plane &gplane,
  ...
  isValid = false;
  if (withinMaxDist) {
    computeModel(vertices, fmParams.defaultTreeRadius,
...
void Cylinder::computeModel(const std::vector<TreeVertex> &landmarkVtxs,
  ...
  pcl::PointIndices::Ptr inliers(
      new pcl::PointIndices);  // inliers indicate the point where the error can
                               // be tolerated, and the sequence number of the
                               // point cloud is recorded
  pcl::SACSegmentation<PointT> seg;  // Create a splitter
  seg.setOptimizeCoefficients(
      true);  // Optional, this setting can choose whether the points displayed
              // in the result plane are split points or the remaining points.
  seg.setModelType(pcl::SACMODEL_LINE);  // Mandatory-set the target geometry
  seg.setMethodType(
      pcl::SAC_RANSAC);  // Segmentation method: random sampling method
  seg.setDistanceThreshold(
      0.25);  // Set the error tolerance range, which is the threshold
  seg.setInputCloud(tree);  // Input point cloud
  seg.segment(*inliers,
              *coefficients);  // Split point cloud, get plane and normal vector
...
void Cylinder::project(const SE3 &tf) {
  ...
  CloudT tmp_pc;
  tmp_pc.points = features;
  pcl::transformPointCloud(tmp_pc, tmp_pc, tf.matrix());
  features = tmp_pc.points;
}