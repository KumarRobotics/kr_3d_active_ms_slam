...
void Tree::view_tree() { write_graphviz(std::cout, graph); }
...
Polygon Tree::get_vertex(const BoostVertexT v) { return graph[v]; }

BoostVertexT Tree::merge_op(BoostVertexT i, BoostVertexT j,
                            const Polygon& data) {
  ...
  if (graph[i].edges.size() > 3) {
    ...
    // TODO: Study the boost graph lib to find a more memory-efficient way of
    // doing this
