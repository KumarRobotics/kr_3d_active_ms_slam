...
void polygonMatching(urquhart::Observation &ref, std::vector<size_t> refIds,
  ...
  for (auto rIdx : refIds) {
    ...
    for (auto tIdx : targIds) {
      ...
      // if tIdx was not matched before and the difference of number of points
...
void linePointMatching(const urquhart::Polygon &A, const urquhart::Polygon &B,
  ...
  do {
    std::vector<double> permutation = {
        B.edgeLengths[chi[0]], B.edgeLengths[chi[1]], B.edgeLengths[chi[2]]};
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    if (unique_matches.find(idx) == unique_matches.end()) {
      ...
      if (idxMatches) {
        idxMatches->push_back(std::make_pair(pAIdx, pBIdx));
      }
    }
  }
}
...
void hierarchyMatching(urquhart::Observation &ref, urquhart::Observation &targ,
  ...
  for (auto tMatch : triangleMatches) {
    ...
    linePointMatching(rT, tT, unique_matches, pointMatches, idxMatches);
  }
}
