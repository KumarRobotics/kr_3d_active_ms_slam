...
// The accumulated lens could be simplified by using the already available edge
// lengths. We implemented this way to follow the original code.
  ...
  while (std::abs(coveredPerim - 1.0) > 0.0001) {
    ...
    if (currLine != 0) {
      d = (std::abs(coveredPerim - normalizedAccumLens[currLine - 1]) /
           std::abs(normalizedAccumLens[currLine] -
                    normalizedAccumLens[currLine - 1]));
...
std::vector<double> centroidDist(PointVector points,
                                 PointVector sampledPoints) {
  vecPtT centroid(2, 0);
  for (vecPtT p : points) {
    centroid[0] += p[0];
...
// based on the official OpenCV tutorial:
...
std::vector<double> invariantFourier(std::vector<double> centroidDesc) {
  ...
  cv::Mat planes[] = {
      matDesc, cv::Mat::zeros(cv::Size(1, centroidDesc.size()), CV_64FC1)};
  cv::Mat complexI;
  cv::merge(planes, 2,
            complexI);  // Add to the expanded another plane with zeros
  cv::dft(complexI,
  ...
  if (magI.isContinuous()) {
    dftDesc.assign((double*)magI.data,
                   (double*)magI.data + magI.total() * magI.channels());
  } else {
    for (int i = 0; i < magI.rows; ++i) {
      dftDesc.insert(dftDesc.end(), magI.ptr<double>(i),
                     magI.ptr<double>(i) + magI.cols * magI.channels());
    }
  }
  return dftDesc;
}
