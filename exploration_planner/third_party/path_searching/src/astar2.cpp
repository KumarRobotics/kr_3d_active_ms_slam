...
void Astar::init(ros::NodeHandle& nh, const EDTEnvironment::Ptr& env) {
  ...
  edt_env_->occ_map_->getRegion(origin_, map_size_3d_);
...
int Astar::search(const Eigen::Vector3d& start_pt, const Eigen::Vector3d& end_pt) {
  ...
  if (edt_env_->occ_map_->getOccupancy(start_pt) == SDFMap::UNKNOWN) ROS_ERROR("Start unknown, but we take unknown as free now!!");
  if (edt_env_->occ_map_->getOccupancy(end_pt) == SDFMap::UNKNOWN) ROS_ERROR("Goal unknown, but we take unknown as free now!!");
  if (edt_env_->occ_map_->getOccupancy(start_pt) == SDFMap::OCCUPIED) ROS_ERROR("Start OCCUPIED, but we take unknown as free now!!");
  if (edt_env_->occ_map_->getOccupancy(end_pt) == SDFMap::OCCUPIED) ROS_ERROR("Goal OCCUPIED, but we take unknown as free now!!");
    ...
    for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
          ...
          // Check safety
          if (!edt_env_->occ_map_->isInBox(nbr_pos)) continue;
          if (edt_env_->occ_map_->getInflateOccupancy(nbr_pos) == 1) {
              // || edt_env_->occ_map_->getOccupancy(nbr_pos) == SDFMap::UNKNOWN) {
            // if (edt_env_->occ_map_->getOccupancy(nbr_pos) == SDFMap::UNKNOWN) ROS_ERROR("A star, nbr unknown");
            continue;
          }
          ...
          for (double l = 0.1; l < len; l += 0.1) {
            ...
            if (edt_env_->occ_map_->getInflateOccupancy(ckpt) == 1) {
                // || edt_env_->occ_map_->getOccupancy(ckpt) == SDFMap::UNKNOWN) {
              safe = false;
              break;
            }
          }
