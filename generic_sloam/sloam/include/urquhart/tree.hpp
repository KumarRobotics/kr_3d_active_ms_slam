...
using BoostGraph =
    adjacency_list<vecS, vecS, bidirectionalS, urquhart::Polygon>;
...
class BFSVisitor : public boost::default_bfs_visitor {
 public:
  ...
  void discover_vertex(BoostVertexT s, const BoostGraph& g) {
    if (s != 0) visited.push_back(s);
  }
  void discover_vertex(BoostVertexT s, const BoostRGraph& g) {
    if (s != 0) visited.push_back(s);
  }
...
class Tree {
 public:
  ...
  // Merges two polygons of the tree. If i or j have more than 3 vertices, they
  // are disconnected from the main structure and their children are connected
  // to the new polygon. They are not deleted to keep the incremental indices
  // (this could be improved to reduce memory consumption).
  ...
  // Gets the children of a vertex. This function reverses the graph and does a
  // search,
  // TODO: this could be simplified by getting the in_edges of v which should be
  // always 1
