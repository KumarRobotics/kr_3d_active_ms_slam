...
void Observation::urquhartTesselation_() {
  ...
  for (size_t leaf : leaves) {
    ...
    if (leafAncIdx != neighAncIdx) {
      ...
      if (leafAncIdx != leaf) p = H->get_vertex(leafAncIdx);
...
Polygon Observation::combinePolygonData_(const Polygon& p, const Polygon& n) {
  ...
  if (same_direction) {
    neighbors.insert(neighbors.end(), p.neighbors.begin(),
                     p.neighbors.end() - 1);
    neighbors.insert(neighbors.end(), n.neighbors.rbegin(),
                     n.neighbors.rend() - 1);
    ...
    edgeLengths.insert(edgeLengths.end(), p.edgeLengths.begin(),
                       p.edgeLengths.end() - 1);
    edgeLengths.insert(edgeLengths.end(), n.edgeLengths.rbegin(),
                       n.edgeLengths.rend() - 1);

  } else {
    neighbors.insert(neighbors.end(), p.neighbors.begin(),
                     p.neighbors.end() - 1);
    neighbors.insert(neighbors.end(), n.neighbors.begin() + 1,
                     n.neighbors.end());
    ...
    edgeLengths.insert(edgeLengths.end(), p.edgeLengths.begin(),
                       p.edgeLengths.end() - 1);
    edgeLengths.insert(edgeLengths.end(), n.edgeLengths.begin() + 1,
                       n.edgeLengths.end());
  }
...
std::vector<EdgeT>::iterator Observation::findEdge_(Polygon& x,
                                                    EdgeT commonEdge) {
  std::vector<EdgeT>::iterator it =
      std::find_if(x.edges.begin(), x.edges.end(), [&commonEdge](EdgeT e) {
        if (e.first == commonEdge.first && e.second == commonEdge.second)
          return True;
        if (e.second == commonEdge.first && e.first == commonEdge.second)
          return True;
        return False;
      });
  return it;
...
Polygon Observation::mergePolygons_(Polygon& p, Polygon& n, EdgeT commonEdge) {
  ...
  */
  ...
  // rotates puts idx as first element, for p we want the common edge to be the
  // last elem we want the element after commonEdge to be the first, i.e.
  // commonEdge is the last element
  ...
  if (n_it == n.edges.end()) return Polygon();
...
void Observation::delaunayTriangulation_(PointVector& points,
  ...
  // the facet ids are confusing, we want to map the good facets to order of
  // appearance start at one because the root is 0
  size_t fIdx = 1;
  ...
  while (k.hasNext()) {
    ...
    // this will only list neighbors that are also a good facet, so we complete
    // with -1
    ...
    // shifting neighbor order to align with the edges (edge[i] will be shared
    // with neighIds[i])
    ...
    for (auto i = 0; i < vtxIds.size(); ++i) {
      int endIdx = i + 1;
      if (endIdx == vtxIds.size()) endIdx = 0;
